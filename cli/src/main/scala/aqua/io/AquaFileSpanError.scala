...
sealed trait AquaFileSpanError {
  def showForConsole: String
}

case class CustomSyntaxError(span: FileSpan, message: String) extends AquaFileSpanError {
...
case class SyntaxError(span: FileSpan, expectations: NonEmptyList[Expectation])
    extends AquaFileSpanError {
