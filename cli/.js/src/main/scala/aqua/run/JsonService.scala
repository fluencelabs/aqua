...
object JsonService {
  def jsonServiceOpt[F[_]: Files: Concurrent]: Opts[F[ValidatedNec[String, JsonService]]] = {
    jsonFromFileOpt("json-service", "Path to file that describes service with JSON result", "j")
      .map(b => b.map {
        case a: ValidatedNec[String, js.Dynamic] => a.andThen{ res =>
          val name = res.name
          val functionsRaw = res.functions
          if (js.isUndefined(name) && js.typeOf(name) != "string")
            invalidNec("No name or it is not a string")
          else if (js.isUndefined(functionsRaw) && !js.Array.isArray(functionsRaw))
            invalidNec("'functions' field should exists and be an array")
          else {
            val functionsV: ValidatedNec[String, List[JsonFunction]] = functionsRaw.asInstanceOf[js.Array[js.Dynamic]].toList.map { f =>
              val fName = f.name
              val fResult = f.result
              if (js.isUndefined(fName) && js.typeOf(name) != "string")
                invalidNec("One of a functions not have a name or it is not a string")
            ...
            functionsV.andThen{ fs =>
              NonEmptyList.fromList(fs).map(fNEL => validNec(JsonService(name.asInstanceOf[String], fNEL))).getOrElse(invalidNec(s"List of functions in '$name' service is empty"))
            }
          }
