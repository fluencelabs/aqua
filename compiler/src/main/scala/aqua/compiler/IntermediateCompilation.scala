...
import aqua.semantics.header.{HeaderSem, HeaderSemRawContext, PickerRawContext}
import cats.data.*
...
object IntermediateCompilation extends Logging {
  ...
  def getAquaCompiler[S[_]: Comonad](
    config: AquaCompilerConf
  ): (HeaderSemRawContext[S], AquaCompiler[RawContext]) = {
    ...
    (new HeaderSemRawContext[S](), new AquaCompiler[RawContext])
  }
  ...
  def compile[F[_]: Monad, E, I: Order, S[_]: Comonad](
    ...
    val (headerSem, compiler) = getAquaCompiler(config)
