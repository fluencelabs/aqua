...
class NamesAlgebra[F[_], Alg[_]](implicit V: InjectK[NameOp[F, *], Alg]) {
  ...
  def constantDefined(name: Name[F]): Free[Alg, Option[Literal[F]]] =
  ...
  def defineConstant(name: Name[F], literal: Literal[F]): Free[Alg, Boolean] =
    Free.liftInject[Alg](DefineConstant(name, literal))
