...
import aqua.model.ValueModel.{Ability, MutableType}
...
case class ArgsCall(args: ProductType, callWith: List[ValueModel]) {
  ...
  lazy val streamArgs: Map[String, VarModel] =
    zipped.collect { case ((name, _: MutableStreamType), MutableType(vr, _)) =>
      name -> vr
    }.toMap
  ...
  lazy val streamToImmutableArgsWithTypes: Map[String, (VarModel, MutableStreamType)] =
    zipped.collect {
      case ((name, _: ImmutableCollectionType), vr@MutableType(_, t)) =>
        name -> (vr, t)
    }.toMap
