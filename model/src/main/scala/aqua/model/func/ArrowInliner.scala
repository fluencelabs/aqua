...
import aqua.raw.ops.{Call, FuncOp, FuncOps, RawTag}
...
object ArrowInliner extends Logging {
  ...
  def handleTag(acc: InlineAcc, tag: RawTag): (InlineAcc, FuncOp.Tree) = {
    InlineAcc.handleTag[InlineAcc](tag).run(acc).value
  }

  def handleTree[S: Exports: Counter: Mangler: Arrows](tree: FuncOp.Tree): State[S, FuncOp.Tree] =
    FuncOp.traverseS(tree, InlineAcc.handleTag(_))
